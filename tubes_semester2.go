...
func tambahPeserta(A *tabPeserta, n *int) {
	fmt.Scan(&(*A)[*n].id, &(*A)[*n].nama, &(*A)[*n].bidangMinat, &(*A)[*n].tanggalDaftar, &(*A)[*n].aktif)
	*n = *n + 1
}
...
func hapusPeserta(A *tabPeserta, n *int) {
	...
	fmt.Println("Semua data dengan ID tersebut telah dihapus")
}
				...
				case 1: 
					...
					fmt.Scan(&jenisSearch)
						...
						if jenisSearch == 1 {
							...
							fmt.Print("Bidang minat yang dicari: ")
							fmt.Scan(&dicari)
							bin = binSearchMinat(data, n, dicari)
							if bin != -1 {
								fmt.Println(data[seq])
							} else {
								fmt.Println("Data tidak ditemukan")
							}
						}
				case 2: 
					...
					fmt.Scan(&jenisSearch)
						...
						if jenisSearch == 1 {
							...
							fmt.Print("Nama yang dicari: ")
							fmt.Scan(&dicari)
							bin = binSearchNama(data, n, dicari)
							if bin != -1 {
								fmt.Println(data[seq])
							} else {
								fmt.Println("Data tidak ditemukan")
